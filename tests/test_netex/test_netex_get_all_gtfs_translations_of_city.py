...
def test_netex_get_all_gtfs_translations_of_city_success():
    """
    Test that if get_operating_city() is set, the GET request is sent
    """
    config.set_operating_city("Sofia")

    mock_entities = [{"id": "urn:ngsi-ld:GtfsTranslation:Sofia:A"}]
    ...
    result = netex_utils.netex_get_all_gtfs_translations_of_city()
