...
def test_netex_get_all_gtfs_calendar_of_a_city_success():
    """
    Test that if config.NETEX_OPERATING_CITY is set, the GET request is sent
    """
    config.NETEX_OPERATING_CITY = "Sofia"

    mock_entities = [{"id": "urn:ngsi-ld:GtfsCalendarRule:Sofia:C1"}]
    ...
    result = netex_utils.netex_get_all_gtfs_calendar_of_a_city()

    assert result == mock_entities

def test_netex_get_all_gtfs_calendar_of_a_city_without_city():
    """
    ...
    with pytest.raises(ValueError):
        netex_utils.netex_get_all_gtfs_calendar_of_a_city()