...
def test_netex_index_transfers_by_trip_pair_success():
    """
    Check that transfers are grouped by (from_trip_id, to_trip_id).
    """
    transfers = [
        {
    ...
    result = netex_utils.netex_index_transfers_by_trip_pair(transfers)

    assert result == {
        ("urn:ngsi-ld:GtfsTrip:Sofia:Trip1", "urn:ngsi-ld:GtfsTrip:Sofia:Trip2"): [transfers[0], transfers[1]],
        ("urn:ngsi-ld:GtfsTrip:Sofia:Trip3", "urn:ngsi-ld:GtfsTrip:Sofia:Trip4"): [transfers[2]]
    }


def test_netex_index_transfers_by_trip_pair_empty_input():
    """
    Check that empty input returns empty dictionary.
    """
    netex_utils.netex_helper_filter_valid_transfers_for_service_journey_interchanges = MagicMock(return_value=[])

    result = netex_utils.netex_index_transfers_by_trip_pair([])

    assert result == {}


def test_netex_index_transfers_by_trip_pair_calls_filter():
    """
